...
export default {
  ...
  methods: {
    ...
    getComment () {
      ...
      let url = ''
      if (this.type === 1) {
        url = '/comment/drama/detail?dramaId='
      } else if (this.type === 0) {
        url = '/comment/chat/detail?chatId='
      }
    ...
    postComment () {
      if (this.loginIn) {
        ...
        var badword = ['他妈的', '有病' , '傻逼' , '智障'];
        var value = _this.textarea;
        var res = '';
        //遍历敏感词数组
        ...
        if (this.type === 1) {
          params.append('dramaId', _this.id)
        } else if (this.type === 0) {
          params.append('chatId', _this.id)
        }
        params.append('userId', _this.userId)
        params.append('type', _this.type)
        // params.append('content', _this.textarea)
