...
<!--        </div>-->
        ...
        <div class="songs-body">
          ...
          <comment :id="dramaId" :type="1"></comment>
        </div>
      </div>
    </div>
  </div>
...
<script>
  ...
  export default {
    ...
    created () {
      this.dramaId = this.tempList.id // 给歌单ID赋值
      this.singers = this.tempList
      this.getSongId() // 获取歌单里面的歌曲ID
      this.getRank(this.dramaId) // 获取评分
    },
    mixins: [mixin],
    methods: {
      // 收集歌单里面的歌曲
      ...
      pushValue () {
        if (this.loginIn) {
          ...
          params.append('dramaId', _this.dramaId)
